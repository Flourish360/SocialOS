...
def sync_instagram_account(db: Session, account) -> tuple[bool, str | None]:
    """Fetch live profile data for one Instagram account and cache it in the DB.

    Returns (True, None) on success, or (False, reason) on failure. The reason
    used to only reach the server log (callers just got a bare bool), so a
    caller-visible failure like "Instagram sync failed: handle=... user_id=..."
    gave no clue why, the real cause (a revoked token, a rate limit, whatever
    Meta actually said) was invisible outside the logs.
    """
    if not account.access_token or not account.platform_user_id:
        return False, "missing access token or platform_user_id"
    ...
    if "error" in data:
        message = data["error"].get("message") or "unknown Graph API error"
        log.warning("Instagram API error for %s: %s", account.id, message)
